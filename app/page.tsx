    ...
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="flex min-h-[calc(100vh-8rem)] flex-col justify-center py-12 sm:py-20">
      <div className="mx-auto w-full max-w-5xl">
        ...
        <motion.header initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }} className="max-w-4xl">
          ...
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-400 sm:text-xl">CBCS Elective Guide turns your learning preferences into a focused, explainable course path — so every credit moves you forward.</p>
        </motion.header>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-2 rounded-full border border-blue-400/20 bg-blue-400/10 px-4 py-2 text-sm font-semibold text-blue-200"><Check className="size-4 text-blue-400" /> Explainable matching</span><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-300"><GraduationCap className="size-4 text-slate-400" /> Built for CBCS</span><span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-300"><MessageSquareQuote className="size-4 text-slate-400" /> Peer Course Reviews</span></div>

        <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-12 flex flex-col gap-6 rounded-3xl border border-blue-400/20 bg-blue-500/10 p-6 shadow-2xl shadow-blue-950/20 sm:flex-row sm:items-center sm:justify-between sm:p-8"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-300">For seniors & alumni</p><h2 className="mt-2 max-w-xl text-2xl font-bold tracking-tight text-white">Already taken these electives?</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-300">Share your review to help juniors make better choices.</p></div><button type="button" onClick={() => setView("testimonial_login")} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-slate-950 transition hover:bg-blue-50"><MessageSquareQuote className="size-4" /> Write a Testimonial</button></motion.section>
        ...
        <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.42 }} className="mt-24 border-t border-white/10 pt-8">
          ...
          <div className="grid gap-4 md:grid-cols-3">{[{ number: "01", title: "Set your direction", text: "Tell us who you are and where you study.", icon: GraduationCap }, { number: "02", title: "Map your preferences", text: "Six quick signals reveal how you learn best.", icon: Command }, { number: "03", title: "See your fit", text: "Explore ranked electives with transparent reasoning.", icon: Sparkles }].map((step, index) => { const Icon = step.icon; return <motion.div key={step.number} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -6 }} transition={{ delay: 0.5 + index * 0.12 }} className="group rounded-3xl border border-white/10 bg-white/[0.04] p-6 shadow-sm shadow-black/20 transition-shadow hover:border-blue-400/30 hover:bg-white/[0.07] hover:shadow-xl hover:shadow-blue-950/30"><div className="flex items-start justify-between"><span className="flex size-11 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-600/20"><Icon className="size-5" /></span><span className="font-mono text-sm font-bold text-blue-600">{step.number}</span></div><h3 className="mt-8 text-lg font-bold text-slate-950">{step.title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{step.text}</p><div className="mt-8 h-1 w-10 rounded-full bg-blue-200 transition-all group-hover:w-16 group-hover:bg-blue-600" /></motion.div>})}</div>
        </motion.section>
