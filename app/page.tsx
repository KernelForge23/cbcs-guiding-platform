    ...
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="flex min-h-[calc(100vh-8rem)] flex-col justify-center py-12 sm:py-20">
      <div className="mx-auto w-full max-w-5xl">
        ...
        <motion.header initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }} className="max-w-4xl">
          <h1 className="max-w-4xl text-5xl font-bold tracking-[-0.045em] text-slate-950 sm:text-6xl lg:text-7xl">CBCS Guiding Platform</h1>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600 sm:text-xl">CBCS Elective Guide turns your learning preferences into a focused, explainable course path — so every credit moves you forward.</p>
        </motion.header>

        <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.28 }} className="mt-12 rounded-3xl border border-slate-200 bg-white p-3 shadow-2xl shadow-blue-950/10 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div className="flex-1 rounded-2xl border border-transparent px-5 py-4 transition focus-within:border-blue-200 focus-within:bg-blue-50/40"><label htmlFor="student-name" className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">Your name</label><input id="student-name" type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full border-0 bg-transparent p-0 text-lg font-semibold text-slate-950 outline-none focus:ring-0" /></div>
            <div className="hidden h-14 w-px bg-slate-200 sm:block" />
            <div className="relative flex-1 rounded-2xl border border-transparent px-5 py-4 transition focus-within:border-blue-200 focus-within:bg-blue-50/40"><label id="student-branch-label" className="mb-2 block text-[11px] font-bold uppercase tracking-[0.14em] text-slate-500">Your branch</label><button id="student-branch" type="button" aria-haspopup="listbox" aria-expanded={isBranchMenuOpen} aria-labelledby="student-branch-label" onClick={() => setIsBranchMenuOpen((open) => !open)} className="flex w-full items-center justify-between gap-3 bg-transparent p-0 text-left text-lg font-semibold text-slate-950 outline-none"><span className={branch ? "" : "text-slate-400"}>{branch || "Select your branch"}</span><ChevronDown className="size-5 text-slate-400" /></button>{isBranchMenuOpen && <div role="listbox" aria-labelledby="student-branch-label" className="absolute left-3 right-3 top-full z-20 mt-3 max-h-72 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl shadow-slate-950/15">{BRANCHES.map((b) => <button key={b} type="button" role="option" aria-selected={branch === b} onClick={() => { setBranch(b); setIsBranchMenuOpen(false); }} className="w-full rounded-xl px-3 py-2.5 text-left text-sm font-medium text-slate-700 transition hover:bg-blue-50 hover:text-blue-700">{b}</button>)}</div>}</div>
        ...
        <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.42 }} className="mt-24 border-t border-slate-200 pt-8">
          ...
          <div className="grid gap-4 md:grid-cols-3">{[{ number: "01", title: "Set your direction", text: "Tell us who you are and where you study.", icon: GraduationCap }, { number: "02", title: "Map your preferences", text: "Six quick signals reveal how you learn best.", icon: Command }, { number: "03", title: "See your fit", text: "Explore ranked electives with transparent reasoning.", icon: Sparkles }].map((step, index) => { const Icon = step.icon; return <motion.div key={step.number} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} whileHover={{ y: -6 }} transition={{ delay: 0.5 + index * 0.12 }} className="group rounded-3xl border border-slate-200 bg-gradient-to-br from-white to-blue-50/50 p-6 shadow-sm transition-shadow hover:shadow-xl hover:shadow-blue-950/10"><div className="flex items-start justify-between"><span className="flex size-11 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-600/20"><Icon className="size-5" /></span><span className="font-mono text-sm font-bold text-blue-600">{step.number}</span></div><h3 className="mt-8 text-lg font-bold text-slate-950">{step.title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{step.text}</p><div className="mt-8 h-1 w-10 rounded-full bg-blue-200 transition-all group-hover:w-16 group-hover:bg-blue-600" /></motion.div>})}</div>
        </motion.section>
