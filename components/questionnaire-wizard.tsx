...
export function QuestionnaireWizard({
  ...
  return (
    <div className="relative min-h-[calc(100vh-8rem)] flex flex-col justify-between py-2 sm:py-4 text-slate-900 dark:text-white bg-white dark:bg-slate-950">
      {/* Top Header & Progress Navigation */}
      <header className="relative z-10 w-full max-w-4xl mx-auto px-4 mb-2 sm:mb-4">
        ...
        <div className="relative w-full h-2 rounded-full bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/50 overflow-hidden">
          <motion.div
          ...
          {WIZARD_QUESTIONS.map((q, idx) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center justify-center h-7 text-[11px] rounded-full font-semibold transition-all ${
                  isCurrent
                    ? "px-3 bg-indigo-600 text-white shadow-sm ring-2 ring-indigo-300 dark:ring-indigo-700"
                    : isAnswered
                    ? "w-7 bg-slate-100 dark:bg-slate-800 border border-indigo-300 dark:border-indigo-500/40 text-indigo-700 dark:text-indigo-300 hover:bg-slate-200 dark:hover:bg-slate-700"
                    : "w-7 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 hover:border-slate-300 dark:hover:border-slate-700"
                }`}
      ...
      <main className="relative z-10 w-full max-w-3xl mx-auto px-4 my-auto">
        <div className="relative rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl dark:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.5)] p-5 sm:p-8 overflow-hidden">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800/60 text-indigo-700 dark:text-indigo-300 text-[10px] sm:text-xs font-bold tracking-wider uppercase">
              ...
              {/* Question Text */}
              <h2 className="font-question text-xl sm:text-2xl lg:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-relaxed sm:leading-snug">
                ...
                {OPTIONS.map((opt) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`group relative flex flex-col items-center justify-center gap-2 p-3 sm:p-4 rounded-2xl border text-center font-semibold text-sm sm:text-[14px] leading-snug transition-all duration-150 cursor-pointer ${
                        isSelected
                    ...
                    >
                      ...
                      <div
                        className={`flex items-center justify-center w-5 h-5 sm:w-6 sm:h-6 rounded-full border transition-all duration-150 shrink-0 ${
                          isSelected
          ...
          <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
            <span>Press <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono text-[9px] text-slate-700 dark:text-slate-300">1-4</kbd> to answer</span>
            <span className="hidden sm:inline">Use <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono text-[9px] text-slate-700 dark:text-slate-300">←</kbd> <kbd className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-mono text-[9px] text-slate-700 dark:text-slate-300">→</kbd> to navigate</span>
          </div>
        </div>
      </main>

      {/* Bottom Footer Actions */}
      <footer className="relative z-10 w-full max-w-3xl mx-auto px-4 mt-4 sm:mt-6 space-y-3">
        {error && (
          <div className="p-3 rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-sm font-medium text-center">
            {error}
          </div>
        )}
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {currentIndex < totalQuestions - 1 ? (
            <button
              ...
              onClick={handleNext}
              disabled={isLoading}
              className="inline-flex items-center gap-2 px-5 py-2 sm:py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-sm font-semibold text-white shadow-md transition disabled:opacity-50"
            >
              <span>Next</span>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-2 sm:py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-sm font-bold text-white shadow-md transition hover:scale-[1.01] active:scale-[0.99] disabled:opacity-40 disabled:scale-100 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
