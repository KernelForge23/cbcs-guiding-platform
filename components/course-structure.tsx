...
type Course = { name: string; department?: string; unavailable?: string };
  ...
  "Engineering Sciences (ESC)": [
    {
      ...
      rule: "Select any 2 subjects (3 credits each) across Sem I and Sem II.",
      courses: [
        { name: "Basics of Civil Engineering", department: "Civil" },
        { name: "Fundamentals of Physical Infrastructure", department: "Civil" },
        { name: "AI for Multidisciplinary Applications", department: "Computer" },
        { name: "Applied Electronics and IOT", department: "Electronics" },
        { name: "Electrical Energy Utilization", department: "Electrical" },
        { name: "Fundamentals of Measurement and Sensors", department: "Instrumentation" },
        { name: "Foundations in Mechanical Engineering", department: "Mechanical" },
        { name: "Nanomaterials", department: "Metallurgy" },
        { name: "Basics of Manufacturing Technology", department: "Manufacturing Science" },
        { name: "Applied Mechanics", department: "Civil" },
        { name: "Fundamentals of Cyber Security", department: "Computer" },
        { name: "Basic Electrical Engineering", department: "Electrical" },
        { name: "Digital Logic Design", department: "Electronics" },
        { name: "Engineering Graphics", department: "Mechanical" },
      ],
    },
  ],
  "Vocational (VSEC)": [
...
function CourseNode({ course }: { course: Course }) {
  const restricted = Boolean(course.department);
  return (
...
export function CourseStructure() {
  ...
  return (
    <section aria-labelledby="curriculum-heading" className="mt-24 border-t border-white/10 pt-16">
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-xs text-slate-400 sm:justify-start sm:gap-6 sm:rounded-full"><span className="inline-flex items-center gap-2"><LockKeyhole className="size-4 text-blue-400" /> Home Branch Restricted <span className="hidden sm:inline">(Students from the offering department cannot take this course).</span></span><span className="inline-flex items-center gap-2"><CheckCircle2 className="size-4 text-emerald-400" /> Open Elective <span className="hidden sm:inline">(Available to all branches).</span></span></div>
    </section>
  );
}
