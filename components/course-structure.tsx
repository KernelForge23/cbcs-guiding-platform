  ...
  return (
    <section aria-labelledby="curriculum-heading" className="mt-24 border-t border-slate-200 pt-16 dark:border-white/10">
      ...
      <AnimatePresence mode="wait"><motion.div key={activeTab} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.2 }} className="mt-8 space-y-5">
        <div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-blue-600 text-sm font-bold text-slate-900 dark:text-white">01</span><span className="text-sm font-semibold text-slate-700 dark:text-slate-300">{activeTab}</span><div className="h-px flex-1 bg-gradient-to-r from-blue-400/60 to-transparent" /></div>
        {categories[activeTab].map((collection, index) => { const id = `${activeTab}-${index}`; const isOpen = openItems.includes(id); return <motion.div layout key={id} className="relative ml-4 border-l border-blue-400/30 pl-7"><span className="absolute -left-1.5 top-7 size-3 rounded-full border-2 border-slate-950 bg-blue-400" aria-hidden="true" /><button type="button" aria-expanded={isOpen} onClick={() => toggleItem(id)} className="flex w-full items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.05] px-5 py-4 text-left transition hover:border-blue-400/40 hover:bg-white/[0.08]"><span><span className="block font-semibold text-slate-900 dark:text-white">{collection.title}</span><span className="mt-1 block text-xs leading-5 text-slate-600 dark:text-slate-400">Constraint: {collection.rule}</span></span><ChevronDown className={`size-5 shrink-0 text-slate-500 transition-transform ${isOpen ? "rotate-180 text-blue-400" : ""}`} /></button><AnimatePresence initial={false}>{isOpen && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><div className="ml-5 border-l border-dashed border-slate-200 dark:border-white/15 py-4 pl-5"><div className="grid gap-3 sm:grid-cols-2">{collection.courses.map((course) => <CourseNode key={course.name} course={course} />)}</div></div></motion.div>}</AnimatePresence></motion.div>; })}
      </motion.div></AnimatePresence>
      <div className="mt-8 flex flex-wrap items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-700 dark:border-white/10 dark:bg-white/[0.03] dark:text-slate-400 sm:justify-start sm:gap-6 sm:rounded-full"><span className="inline-flex items-center gap-2"><LockKeyhole className="size-4 text-blue-400" /> Home Branch Restricted <span className="hidden sm:inline">(Students from the offering department cannot take this course).</span></span><span className="inline-flex items-center gap-2"><CheckCircle2 className="size-4 text-emerald-400" /> Open Course <span className="hidden sm:inline">(Available to all branches).</span></span></div>
    </section>
  );
}
